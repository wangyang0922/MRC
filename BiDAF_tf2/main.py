...
class BiDAF:
    ...
    def build_model(self):
        ...
        embedding_matrix = load_embedding_matrix(max_vocab_size=self.max_features)
        embedding_layer = tf.keras.layers.Embedding(self.max_features,
                                                    self.emb_size,
                                                    weights=[embedding_matrix],
                                                    embeddings_initializer='uniform',
                                                    )
        cnn_layer = tf.keras.layers.Conv1D(self.emb_size,
                                           5,
                                           activation='tanh',
                                           trainable=True)

        cemb = tf.keras.layers.Concatenate(axis=-1)([cnn_layer, embedding_layer])
        qemb = tf.keras.layers.Concatenate(axis=-1)([cnn_layer, embedding_layer])
